...
| **jev-context-curator.ts** | System One attention routing: at each turn boundary, Jev judges whether past tool outputs (≥2 turns old, ≥1500 chars) are still needed for the session goal; stale exploration output is replaced in model context by an append-only `context_edit` stub — raw history stays intact. `jev_recall` restores any stub verbatim; the goal is seeded from the first user prompt and re-pinnable by the model (`pin_goal` — after reading a linked ticket, on direction changes) or manually (`/goal`); `/curator` shows stats or toggles. Calibrated gate (p≥0.85, conf≥0.65) fires only on unambiguous junk (listings, config dumps, superseded greps); everything borderline stays. Fail-open on Jev errors; `JEVCURATOR=0` kill switch; decisions audit to `~/.pi/agent/jev-decisions/jev-curator.jsonl`. |
