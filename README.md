...
| **jev-context-curator/** | Goal-quality-first context manager (V3; directory extension — `index.ts` + the design doc inside). Modes via `JEVCURATOR_MODE`: default `v2` keeps the proven economics layer (outputs >25k capped to head/tail before first exposure; median-of-3 Jev verdicts batch truncate/stub edits, held until saved mass, pressure, or age; `jev_recall` pages raw back verbatim; `pin_goal`/`/goal`; `/curator` stats). `shadow-quality` adds V3 without edits: a versioned **GoalSpec** (user objective + criteria/constraints/plan/facts/open questions, immutable objective, `amend_goalspec` tool), Jev evidence-role classification (active/evidence/background/irrelevant + source type + GoalSpec links), type-aware extract proposals (log line-scoring with deterministic ERROR/summary retention, code/doc ranges, listing matches), and a batched **frontier verifier** at turn_end that approves retainFull/useExtract/indexOnly — uncertainty always retains full. `evidence` additionally emits verifier-approved extracts for log/listing sources; `quality` extends to code/doc and makes compaction carry the full GoalSpec + evidence ledger (with recall ids) into the frontier-generated summary. Every condensed source lands in a searchable ledger with `curator_find` (Jev rerank vs GoalSpec) + `jev_recall` paging as the no-loss contract. Fail-open everywhere; `JEVCURATOR=0` kill switch; audit in `~/.pi/agent/jev-decisions/jev-curator.jsonl` + `jev-curator-v3-shadow.jsonl`. |
