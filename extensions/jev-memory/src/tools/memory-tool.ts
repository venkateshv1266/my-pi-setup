...
export interface MemoryToolHooks {
  /** Notified at the SQLite-reconcile mutation-observer choke point with the
   * number of entries a successful store mutation added (0 for pure shrinks). */
  onSuccessfulAdds?: (
    store: MemoryStore,
...
export function registerMemoryTool(
  ...
  const attachMutationObserver = (candidate: MemoryStore | null, isProjectStore = false): void => {
    ...
    candidate.setMutationObserver((target, entries, addedEntries) => {
      if (addedEntries > 0) {
        hooks.onSuccessfulAdds?.(
          candidate,
          target,
  ...
  const executeAction = async (
    ...
    switch (action) {
      case "add":
        ...
        // JEVADMIT gates only adds; replace/remove shrink the store and are never gated.
        if (jevConfig.enabled && jevConfig.admission.enabled) {
          const scopeEntries = scopeEntriesForTarget(rawTarget, store, activeProjectStore);
          const exactDuplicate = isExactDuplicate(content, scopeEntries);
          const state: AdmissionState = {
            content,
            exact_duplicate: exactDuplicate,
            candidates: exactDuplicate ? [] : selectAdmissionCandidates(content, scopeEntries),
          };
          const startedAt = Date.now();
          // Exact duplicates block deterministically without spending a Jev call.
          ...
          if (gate.verdict === "block") {
            const error = gate.reason ?? "jev-memory admission: blocked";
...
This action-specific tool accepts only the parameters listed in its schema.`;
  ...
  const registerActionTool = (
    ...
    pi.registerTool({
      ...
      parameters,
      async execute(_toolCallId, params, signal, _onUpdate, ctx?: { cwd?: string; ui?: { notify?: (message: string, level?: "info" | "warning" | "error") => void } }) {
        if (bindProjectFromCwd && ctx?.cwd) {
          await bindProjectFromCwd(ctx.cwd);
        }
        return executeAction(action, params as MemoryToolParams, signal, ctx?.ui?.notify?.bind(ctx.ui));
      },
    });
  };
