...
/** Parse the `<!-- created=…, last=… -->` metadata comment of a raw Markdown entry. */
export function parseMetadataComment(raw: string): { text: string; created: string; lastReferenced: string; project: string | null } {
...
export function reconcileMarkdownMemoryScope(
  ...
  try {
    ...
    if (isFts5QueryError(err)) {
      ...
      console.warn(`[pi-jev-memory] FTS5 search index error during markdown sync (search may be stale; run /memory-sync-markdown): ${detail}`);
