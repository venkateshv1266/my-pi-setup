...
export class MemoryStore {
  ...
  private mutationObserver: ((target: "memory" | "user" | "failure", entries: string[], addedEntries: number) => Promise<string | null | undefined>) | null = null;
  private addedSinceFinalize: Partial<Record<"memory" | "user" | "failure", number>> = {};
  ...
  setMutationObserver(
    fn: (target: "memory" | "user" | "failure", entries: string[], addedEntries: number) => Promise<string | null | undefined>,
  ): void {
    this.mutationObserver = fn;
  }

  /** Counts entries a successful mutation added; flushed to the mutation
   * observer at finalization so write-frequency triggers see real adds. */
  private noteAddedEntries(target: "memory" | "user" | "failure", count = 1): void {
    this.addedSinceFinalize[target] = (this.addedSinceFinalize[target] ?? 0) + count;
  }

  // ─── Path helpers ───

  private get memoryDir(): string {
    return this.config.memoryDir ?? path.join(AGENT_ROOT, "jev-memory");
  }
  ...
  async applyMutationPlan(
    ...
    return this.runTargetMutation(target, async (markMutation) => {
      ...
      await this.saveToDisk(target);
      if (plannedAddCount > 0) this.noteAddedEntries(target, plannedAddCount);
      markMutation();
  ...
  private async finalizeTargetMutation(
    ...
    const addedEntries = this.addedSinceFinalize[target] ?? 0;
    delete this.addedSinceFinalize[target];

    if (!this.mutationObserver) return finalized;

    const warning = await this.mutationObserver(target, [...state.entries], addedEntries);
    ...
    return withMarkdownMutationLock(storagePath, async () => {
      for (let attempt = 0; ; attempt++) {
        ...
        try {
          ...
          // The retried mutation re-counts its own adds; drop the aborted
          // attempt's count so one logical add never tallies twice.
          delete this.addedSinceFinalize[target];
