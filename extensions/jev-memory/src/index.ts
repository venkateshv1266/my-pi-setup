// jev-memory — vendored from pi-hermes-memory@0.9.8 (MIT); Jev layer added in T2-T4.
/**
...
import { triggerConsolidation, registerConsolidateCommand, runScheduledConsolidation } from "./handlers/auto-consolidate.js";
...
import { rehomeLegacyStore } from "./store/store-rehome.js";
...
export default function (pi: ExtensionAPI) {
  ...
  const jevConfig = resolveJevConfig();

  const agentRoot = AGENT_ROOT;
  const legacyGlobalDir = path.join(agentRoot, "memory");
  const defaultGlobalDir = path.join(agentRoot, "jev-memory");
  ...
  // ── 2b. JEVCONSOLIDATE scheduled trigger ──
  // Counts successful adds per target at the mutation-observer choke point;
  // every intervalWrites adds fire a fire-and-forget typed consolidation for
  // that target. Never blocks the triggering add.
  const scheduledConsolidationAbort = new AbortController();
  pi.on("session_shutdown", () => scheduledConsolidationAbort.abort());
  const writesSinceConsolidation = new Map<"memory" | "user" | "failure" | "project", number>();
  const noteSuccessfulAdds = (
    targetStore: MemoryStore,
    target: "memory" | "user" | "failure",
    toolTarget: "memory" | "user" | "failure" | "project",
    added: number,
  ) => {
    if (added <= 0 || !jevConfig.enabled || !jevConfig.consolidation.enabled) return;
    const count = (writesSinceConsolidation.get(toolTarget) ?? 0) + added;
    if (count < jevConfig.consolidation.intervalWrites) {
      writesSinceConsolidation.set(toolTarget, count);
      return;
    }
    writesSinceConsolidation.delete(toolTarget);
    if (scheduledConsolidationAbort.signal.aborted) return;
    void (async () => {
      try {
        const result = await runScheduledConsolidation(
          ...
          target,
          toolTarget,
          scheduledConsolidationAbort.signal,
          config.consolidationTimeoutMs,
          jevConfig,
        );
        if (result.deferred) {
          console.info(`⏳ Scheduled consolidation for '${toolTarget}' skipped: another session holds the consolidation lock`);
        } else if (!result.consolidated) {
          console.info(`ℹ️ Scheduled consolidation for '${toolTarget}' stopped: ${result.error ?? "nothing to consolidate"}`);
        }
      } catch (err) {
        console.warn(`⚠️ Scheduled consolidation for '${toolTarget}' failed: ${err instanceof Error ? err.message : String(err)}`);
      }
    })();
  };

  // ── 3. Register action-specific memory write tools with SQLite sync ──
  configureMemoryToolProjectStore = registerMemoryTool(pi, store, projectStoreRef, dbManager, projectNameRef, bindProjectFromCwd, jevConfig, {
    onSuccessfulAdds: noteSuccessfulAdds,
  });
  ...
  setupSessionFlush(pi, store, projectStoreRef, config, dbManager, projectNameRef, jevConfig);
  ...
  registerConsolidateCommand(pi, store, config.consolidationTimeoutMs, projectStoreRef, projectNameRef, config, dbManager, {
    onTargetConsolidated: (toolTarget) => writesSinceConsolidation.delete(toolTarget),
  }, jevConfig);

  // ── 8. Setup correction detection ──
  setupCorrectionDetector(pi, store, projectStoreRef, config, dbManager, projectNameRef, jevConfig);
  ...
  registerMemorySearchTool(pi, dbManager, jevConfig);
  registerIndexSessionsCommand(pi, config, globalDir);
