...
// Resolve the path to pi-jev-memory's own extension entry point.
...
// pi-jev-memory's own package.json declares) and match sibling package
...
// default: packages installed alongside pi-jev-memory's own package
// (the common npm-managed-extensions layout), and AGENT_ROOT's npm
// directory (covers pi-jev-memory being loaded from elsewhere, e.g. a
