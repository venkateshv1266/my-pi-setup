...
import { buildTurnDigest, GATE_CALL_DEADLINE_MS, pregateGate } from "../jev/gates.js";
import { PREGATE_QUESTION } from "../jev/questions.js";
...
export function setupBackgroundReview(
  ...
  pi.on("turn_end", async (event, ctx) => {
    ...
    const runReview = async (): Promise<void> => {
      ...
      // JEVPREGATE: one gate call before the full review prompt is assembled.
      // Skipping counts as a completed quiet review — the nudge counters were
      // already reset above and activeReview clears in .finally().
      if (jevConfig.enabled && jevConfig.pregate.enabled) {
        const startedAt = Date.now();
        const answers = await jevCall(
          { turn_digest: buildTurnDigest(parts) },
          PREGATE_QUESTION,
          { budget: new CallBudget(1, Date.now() + GATE_CALL_DEADLINE_MS) },
        );
        if (sessionCancelled()) return;
        const gate = pregateGate(answers, jevConfig);
        if (jevConfig.audit.enabled) {
          appendAudit({
            ts: new Date().toISOString(),
            decision: "pregate",
            outcome: gate.run ? "run" : "skip",
            degraded: gate.degraded || undefined,
            scores: gate.worthReview !== undefined ? { worth_review: gate.worthReview } : undefined,
