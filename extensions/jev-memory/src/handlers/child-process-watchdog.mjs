...
if (!cancellationPath || !command || !Number.isFinite(timeoutMs) || timeoutMs <= 0) {
  process.stderr.write("pi-jev-memory watchdog: invalid invocation\n");
  process.exit(2);
}
...
const timeout = setTimeout(() => {
  timedOut = true;
  process.stderr.write(`[pi-jev-memory] child timed out after ${timeoutMs}ms; terminating process tree\n`);
  terminateTree();
}, timeoutMs);
timeout.unref();
...
const cancellationPoll = cancellationPath === "-" ? undefined : setInterval(() => {
  ...
  cancelled = true;
  process.stderr.write("[pi-jev-memory] child cancellation requested; terminating process tree\n");
  terminateTree();
}, 25);
...
child.once("error", (error) => {
  ...
  process.stderr.write(`pi-jev-memory watchdog: ${error.message}\n`);
