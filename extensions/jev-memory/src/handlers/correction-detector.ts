...
import { buildCorrectionState, correctionGate, GATE_CALL_DEADLINE_MS, type CorrectionGateResult } from "../jev/gates.js";
import { CORRECTION_QUESTIONS } from "../jev/questions.js";
...
export function setupCorrectionDetector(
  ...
  pi.on("turn_end", async (event, ctx) => {
    ...
    try {
      ...
      // JEVCORRECT: adjudicate the regex match before any save prompt runs.
      // A skip also suppresses the failure-memory save below (regex false positive).
      let jevTargetHint: CorrectionGateResult["target"];
      if (jevConfig.enabled && jevConfig.correction.enabled) {
        const startedAt = Date.now();
        const answers = await jevCall(
          buildCorrectionState(parts),
          CORRECTION_QUESTIONS,
          { budget: new CallBudget(1, Date.now() + GATE_CALL_DEADLINE_MS) },
        );
        const gate = correctionGate(answers);
        if (jevConfig.audit.enabled) {
          appendAudit({
            ts: new Date().toISOString(),
            decision: "correction",
            outcome: gate.save ? (gate.degraded ? "degraded" : "allow") : "skip",
            scores: gate.isCorrection !== undefined ? { is_correction: gate.isCorrection } : undefined,
            latency_ms: Date.now() - startedAt,
          });
        }
        if (!gate.save) return;
        jevTargetHint = gate.target;
      }
      ...
      // Jev hint only — the LLM still owns the final op.
      if (jevTargetHint) {
        promptBody.push("", `Jev adjudicated target: '${jevTargetHint}' — use it unless clearly wrong.`);
      }
